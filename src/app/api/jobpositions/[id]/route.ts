...
export async function GET(_: Request, ctx: Ctx) {
  ...
  const endpoint = resolveEndpoint((API_ENDPOINTS as any)?.jobpositions?.byId, (x) => `/jobpositions/${x}`, id);
...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  // update endpoint (ej: PATCH /jobpositions/:id)
  const endpoint = resolveEndpoint((API_ENDPOINTS as any)?.jobpositions?.update, (x) => `/jobpositions/${x}`, id);
