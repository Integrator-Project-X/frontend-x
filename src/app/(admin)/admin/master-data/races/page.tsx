...
function buildHref(q: string, status: "all" | "active" | "inactive") {
  ...
  return `/admin/master-data/races?${params.toString()}`;
}
...
export default async function RacesPage({ searchParams }: Props) {
  ...
  async function deactivateAction(formData: FormData) {
    ...
    if (!id) return;
    await deactivateRace(id);
    revalidatePath("/admin/master-data/races");
  }
  ...
  <div className="space-y-6">
    <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
      <div className="space-y-1">
        <h1 className="text-2xl font-semibold text-slate-800">Races</h1>
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription className="text-slate-500">
          Search by race name + filter by status.
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <form
          ...
          action="/admin/master-data/races"
          method="GET"
        >
    ...
    <Card>
      ...
      <CardContent>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((r) => (
                <TableRow key={r.id} className="transition hover:bg-slate-50">
                  ...
                  <TableCell className="font-medium text-slate-800">{r.raceName}</TableCell>
        ...
        <p className="mt-3 text-xs text-slate-400">
          Backend: <span className="font-mono">GET /races</span> · action{" "}
          <span className="font-mono">PATCH /races/:id</span> ·{" "}
          <span className="font-mono">PATCH /races/:id/deactivate</span>
        </p>
      </CardContent>
    </Card>
  </div>
