...
export async function GET(_: Request, ctx: Ctx) {
  ...
  const endpoint = resolveEndpoint((API_ENDPOINTS as any)?.personal?.byId, (x) => `/personal/${x}`, id);
...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const endpoint = resolveEndpoint((API_ENDPOINTS as any)?.personal?.update, (x) => `/personal/${x}`, id);
