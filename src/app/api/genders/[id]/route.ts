...
export async function GET(_: Request, ctx: Ctx) {
  ...
  const res = await fetch(`${BASE_URL}${API_ENDPOINTS.genders.byId(id)}`, {
    method: "GET",
...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const res = await fetch(`${BASE_URL}${API_ENDPOINTS.genders.update(id)}`, {
    method: "PATCH",
    headers: {
...
export async function DELETE(_: Request, ctx: Ctx) {
  ...
  const res = await fetch(`${BASE_URL}${API_ENDPOINTS.genders.delete(id)}`, {
    method: "DELETE",
    headers: { Accept: "application/json", ...(await getAuthHeader()) },
    cache: "no-store",
  });
